...
| `desktop.{hpp,cpp}` | Linux: write `lexe-<id>.desktop` (Exec=`lexe run <id>`), install icons to hicolor, MIME XML, best-effort `update-desktop-database`/`update-mime-database`; also `integrate_runtime()` used by packaging to register `application/x-lexe` for the runtime itself. Writes to the XDG data dirs, or — when `LEXE_HOME` is set — to a **confined** tree no desktop scans (`Paths::desktop_scope()`, FORMAT §9); results carry `visible_to_desktop` + a `note`, so a confined write is never reported as a registration. Windows: every function is a recorded no-op (returns `skipped`) so core tests run anywhere |
