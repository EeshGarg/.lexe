...
TEST_CASE("the empty state offers both ways in, and promises no shortcut") {
    ...
    // Regression: this named the retired `lexe-installer` binary, so the
    // empty state told the user to run a command that does not exist.
    CHECK(contains(text.command, "lexe-ui"));
    CHECK_FALSE(contains(text.command, "lexe-installer"));
